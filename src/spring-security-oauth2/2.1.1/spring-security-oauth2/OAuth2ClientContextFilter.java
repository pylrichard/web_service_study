...
public class OAuth2ClientContextFilter implements Filter, InitializingBean {
	...
	public void doFilter(ServletRequest servletRequest,
		...
		try {
			//此处设置断点，在浏览器中输入localhost:8080/book/1
			//第1次触发断点时servletRequest为localhost:8080/book/1，发现用户没有登录
            //
			//第2次触发断点时servletRequest为localhost:8080/login，发送请求到授权服务器，重定向URI让用户进行授权，见AbstractAuthenticationProcessingFilter.doFilter
			//
			//用户授权成功后，授权服务器会发送授权码到第三方应用服务器，回调在向授权服务器申请时提供的URI
			//既是第3次触发断点时servletRequest为localhost:8080/login+授权码
			//看AuthorizationCodeAccessTokenProvider.obtainAccessToken.retrieveToken
			chain.doFilter(servletRequest, servletResponse);
		} catch (IOException ex) {
			throw ex;
		} catch (Exception ex) {
			//捕获OAuth2RestTemplate.getAccessToken中抛出的异常
			// Try to extract a SpringSecurityException from the stacktrace
			Throwable[] causeChain = throwableAnalyzer.determineCauseChain(ex);

			//UserRedirectRequiredException.redirectUri既是application.properties中
			//配置项security.oauth2.client.user-authorization-uri的值
			//如https://github.com/login/oauth/authorize
			//在AuthorizationCodeAccessTokenProvider.getRedirectForAuthorization中设置
	...
	protected void redirectUser(UserRedirectRequiredException e,
		...
		//在AuthorizationCodeAccessTokenProvider.getRedirectForAuthorization中构建参数
		...
		//向授权服务器发送请求，请求包含client id(application.properties文件配置项)，redirect uri(localhost:8080/login)
		//授权服务器返回授权码，见OAuth2ClientContextFilter.doFilter
