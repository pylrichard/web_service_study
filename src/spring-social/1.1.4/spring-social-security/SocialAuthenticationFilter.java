...
public class SocialAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
		...
		if (!authProviders.isEmpty() && authProviderId != null && authProviders.contains(authProviderId)) {
			...
			if (auth == null) {
				/*
					OAuth2Template.postForAccessGrant()返回的是text/html触发异常，返回auth = null
					此处异常在AbstractAuthenticationProcessingFilter.doFilter()中捕获，进入认证失败处理流程
				*/
	...
	private Authentication attemptAuthService(final SocialAuthenticationService<?> authService, final HttpServletRequest request, HttpServletResponse response) 
		...
		/*
			调用OAuth2AuthenticationService.getAuthToken()
		 */
