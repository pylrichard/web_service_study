package com.bd.java_se_study.algorithm;
...
public class AlgorithmComplexityAnalysis {
    ...
    /**
     * n为数组array的大小，则最坏情况下需要比较n次以得到最大值，所以算法复杂度为O(n)
     */
    ...
    /**
     * n为数组array的大小，则基本步骤的执行数量约为n*(n-1)/2，所以算法复杂度为O(n2)
     */
    ...
    /**
     * 给定规模n和m，则基本步骤的执行数量为n*m，所以算法复杂度为O(n2)
     */
    ...
    /**
     * 给定规模n，则基本步骤的执行数量约为n*n*n ，所以算法复杂度为O(n3)
     */
    ...
    /**
     * 给定规模n，则基本步骤的执行数量为2的n次方，所以算法复杂度为O(2n)
     */
    ...
    /**
     * 实际只有前两个计算结果有用，使用中间变量来存储，就不用创建数组以节省空间。同样算法复杂度优化为O(n)
     */
